...
#include "LIBRERIA_LCD.h"  //librería LCD
#include "LIBRERIA_ADC.h"  //librería conversor ADC
     ...
     switch (opcion){
        ...
        case 3:  LCD_Cmd(1);
                 ...
                 do{
                     ...
                     oxígeno = conver4*0.0977;// Linealiza con el rango deseado
                     sprintf (aux,"Oxígeno:%3.1f",oxígeno);// convierte
...
void __interrupt()nombre(){
    if(PIR1bits.RCIF==1){ // Bandera de la interrupcion 
        ...
        // Condición if para las diferentes opciones
        if(t=='1'||t=='2'||t=='3'||t=='4'||t=='5'||t=='6'){
        UART_Wrrite_Chr(' ');
        UART_Wrrite_Chr('O');
        UART_Wrrite_Chr('K');
        UART_Wrrite_Chr(10);// salto de línea
        UART_Wrrite_Chr(13);// Ubicar el cursor en la última posición
        }
        else{
        UART_Wrrite_Chr(' ');
        UART_Wrrite_Chr('E');
...
void main(void) {
    ...
    ADC_Init(_TAD_4,_TOSC_4); // inicializa ADC y ajusta tiempos de adquisición y oscilación
    
    INTCONbits.GIE=1; // configuración interrupciones globales
    INTCONbits.PEIE=1; //configuración periféricos
    
    PIR1bits.RCIF=0; //configuración bandera
    PIE1bits.RCIE=1; //configuración bandera
    
    UART_Init(9600); // inicializa UART y configura baudeaje
    
    serial(); // se incluye el método
    
    while(1){
        
        MenuP(); // se incluye el método
    }
